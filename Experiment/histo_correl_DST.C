...
void histo_correl_DST()
{
  ...
  for (UInt_t i=0; i<nevents; i++)
	...
	{
	  if (ident[j] == FRAGMENT) // aky fragmnet
	    index[count++]=j;
	  if (ident[j] == FRAGMENT_IMP) // aky fragment s akym impulsnom
	    {
      ...
      if ( (count > 1) && (count <= MAX_NUM_FRAG) ) // how many fragment
	{
	  ...
	  for (UShort_t k=0; k < count-1; k++)
	    for (UShort_t l=k+1; l <= count-1; l++)
	      h_true->Fill( mix(px[index[k]],py[index[k]],pz[index[k]],px[index[l]],py[index[l]],pz[index[l]]) );
      ...
      while (count_back_events < nevents_back)
	{
	  ...
	  while (topology_e < m)
	    {
	      ...
	      oxy_DST->GetEntry( n_b_event );
	      count=0;
	      for (UShort_t j=0; j<nchar; j++) // every fragment
		{
		  if (ident[j] == FRAGMENT) // aky fragmnet
		    {
		      tot_p_back[count] = sqrt(px[j]**2+py[j]**2+pz[j]**2);
		      index[count++]=j;
		    }
		  if (ident[j]==FRAGMENT_IMP) // aky fragment s akym inpulsom
		    {
		      tot_p = sqrt(px[j]**2+py[j]**2+pz[j]**2);
		      if ( (tot_p > MIN_IMP) && (tot_p < MAX_IMP) )
			{
	  ...
	  if (to_same_event == 0)
		...
		for (UShort_t l=k+1; l <= m-1; l++)
		  h_back->Fill( mix(px_back[k],py_back[k],pz_back[k],px_back[l],py_back[l],pz_back[l]) );
